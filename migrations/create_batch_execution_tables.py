...
def upgrade():
    ...
    try:
        with get_db_session() as session:
            ...
            batch_executions_exists = session.execute(check_batch_executions).scalar()
            batch_execution_details_exists = session.execute(check_batch_execution_details).scalar()

            if batch_executions_exists and batch_execution_details_exists:
                logger.info("batch_executionsとbatch_execution_detailsテーブルは既に存在します。")
                return
            ...
            if not batch_executions_exists:
                ...
                # インデックス作成
                session.execute(text("CREATE INDEX idx_batch_executions_status ON batch_executions(status)"))
                session.execute(text("CREATE INDEX idx_batch_executions_batch_type ON batch_executions(batch_type)"))
                session.execute(text("CREATE INDEX idx_batch_executions_start_time ON batch_executions(start_time)"))
            ...
            if not batch_execution_details_exists:
                ...
                # インデックス作成
                session.execute(text("CREATE INDEX idx_batch_execution_details_batch_id ON batch_execution_details(batch_execution_id)"))
                session.execute(text("CREATE INDEX idx_batch_execution_details_status ON batch_execution_details(status)"))
                session.execute(text("CREATE INDEX idx_batch_execution_details_stock_code ON batch_execution_details(stock_code)"))
                session.execute(text("CREATE INDEX idx_batch_execution_details_batch_stock ON batch_execution_details(batch_execution_id, stock_code)"))
...
def downgrade():
    ...
    try:
        with get_db_session() as session:
            ...
            session.execute(text("DROP TABLE IF EXISTS batch_execution_details CASCADE"))
            logger.info("batch_execution_detailsテーブルを削除しました。")

            session.execute(text("DROP TABLE IF EXISTS batch_executions CASCADE"))
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 2:
        print("使用法: python create_batch_execution_tables.py [upgrade|downgrade]")
        sys.exit(1)
