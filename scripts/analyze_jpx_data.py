...
def analyze_jpx_data(file_path: str) -> None:
    ...
    if "コード" in df.columns:
        ...
        result = "✓ OK" if max_code_length <= 10 else f"❌ NG (最大{max_code_length}文字)"
    ...
    if "銘柄名" in df.columns:
        ...
        result = "✓ OK" if max_name_length <= 100 else f"❌ NG (最大{max_name_length}文字)"
    ...
    if "市場・商品区分" in df.columns:
        ...
        result = "✓ OK" if max_market_length <= 50 else f"❌ NG (最大{max_market_length}文字)"
    ...
    if "33業種区分" in df.columns:
        ...
        result = "✓ OK" if max_sector_length <= 100 else f"❌ NG (最大{max_sector_length}文字)"
...
if __name__ == "__main__":
    ...
    if not Path(file_path).exists():
        ...
        print("curl -L -o data_j.xls \"https://www.jpx.co.jp/markets/statistics-equities/misc/tvdivq0000001vg2-att/data_j.xls\"")
        sys.exit(1)
