...
class TestAPIE2E:
    ...
    def test_stocks_api_endpoints(self, app_server):
        ...
        # Arrange (準備) - POST /api/stocks (テストデータ作成)
    ...
    def test_individual_stock_data_reading(self, app_server):
        ...
        if data["data"]:
            # Arrange (準備) - 最初のデータのIDを使用してテスト
            stock_id = data["data"][0]["id"]

            # Act (実行) - 1. 有効なIDでの個別データ取得
    ...
    def test_data_deletion_with_validation(self, app_server):
        ...
        # Act (実行) - 2. 負の数のIDでの削除テスト（Flaskルーティングでは負の数も404になる）
        ...
        # Act (実行) - 3. 非常に大きなIDでの削除テスト（有効な数値IDなのでAPIエンドポイントに到達）
