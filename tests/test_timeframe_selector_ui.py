...
class TestTimeframeSelectorUI:
    ...
    def test_css_styles_exist(self):
        ...
        # レスポンシブデザインの確認
        assert '@media (max-width: 768px)' in css_content, "タブレット用メディアクエリが見つかりません"
        assert '@media (max-width: 480px)' in css_content, "モバイル用メディアクエリが見つかりません"
    ...
    def test_accessibility_attributes(self):
        ...
        period_select = soup.find('select', {'id': 'period'})
        assert period_select.get('aria-label') is not None, "aria-label属性が見つかりません"
        assert period_select.get('aria-describedby') is not None, "aria-describedby属性が見つかりません"
    ...
    def test_form_validation_integration(self):
        ...
        assert 'validateTimeframeSelection' in js_content, "時間軸バリデーションが統合されていません"
...
class TestTimeframeSelectorConfiguration:
    ...
    def test_timeframe_options_completeness(self):
        ...
        for optgroup in optgroups:
            ...
            assert len(options) > 0, f"optgroup '{optgroup.get('label')}' にオプションがありません"
