...
class TestAPIE2E:
    ...
    def test_fetch_data_endpoint_valid_symbol(self, app_server):
        ...
        # レスポンスの確認
        assert response.status_code in [200, 400, 500]  # APIの実装によって異なる
    ...
    def test_data_reading_endpoints(self, app_server):
        ...
        # 4. 日付範囲フィルタリングテスト
        response = requests.get(f"{app_server}/api/stocks?start_date=2024-01-01&end_date=2024-12-31")
        assert response.status_code == 200
        data = response.json()
        assert data["success"] is True
        ...
        # 6. 無効な日付形式テスト
        response = requests.get(f"{app_server}/api/stocks?start_date=invalid-date")
        assert response.status_code == 400
        data = response.json()
        assert data["success"] is False
        assert data["error"] == "VALIDATION_ERROR"
    ...
    def test_individual_stock_data_reading(self, app_server):
        ...
        if data["data"]:
            ...
            assert individual_data["success"] is True
    ...
    def test_data_deletion_endpoints(self, app_server):
        ...
        create_response = requests.post(
            f"{app_server}/api/stocks",
            json=test_stock_data
        )
        ...
        if create_response.status_code == 201:
            ...
            # 1. 有効なIDでの削除テスト
            delete_response = requests.delete(f"{app_server}/api/stocks/{created_id}")
            ...
            get_response = requests.get(f"{app_server}/api/stocks/{created_id}")
            ...
            # 既存のデータを取得
            response = requests.get(f"{app_server}/api/stocks?symbol=TEST.T&limit=1")
            if response.status_code == 200:
                data = response.json()
                if data["data"]:
                    ...
                    # 既存データの削除テスト
                    delete_response = requests.delete(f"{app_server}/api/stocks/{existing_id}")
                    assert delete_response.status_code == 200
                    delete_data = delete_response.json()
                    assert delete_data["success"] is True
