...
**JWT認証（`app/api/dependencies/auth.py`）**:
...
**非同期DB接続（`app/api/dependencies/database.py`）**:
...
**サービスインスタンス提供（`app/api/dependencies/services.py`）**:
...
**レート制限デコレータ（`app/api/decorators/rate_limit.py`）**:
...
**バリデーション関数（`app/api/validators/common.py`）**:
...
**統一エラーハンドリング（`app/api/error_handlers.py`）**:
...
**Pydanticスキーマ（`app/schemas/responses.py`）**:
