...
GitHub Actions を中心とした品質チェックの役割と参照先を示します。詳細なワークフローや運用手順、トラブルシューティングは専用ドキュメントに移動しています。

- 参考ドキュメント:
  - [CI/CD パイプライン概要](../ci-cd/pipeline_overview.md)
  - [CI/CD トラブルシューティング](../ci-cd/troubleshooting.md)

このドキュメントでは、GitHub Actions による品質チェックが PR マージ前の必須要件であることのみを扱います。詳細は上記ドキュメントを参照してください。
