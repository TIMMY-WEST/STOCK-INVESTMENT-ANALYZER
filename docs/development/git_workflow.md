...
CI/CD（GitHub Actions 等）の詳細な設定やワークフローは本ドキュメントでは記載していません。
CI/CD のパイプライン設計、ワークフロー、運用手順、トラブルシューティングについては専用ドキュメントを参照してください。

- 主要ドキュメント:
  - [CI/CD パイプライン概要](../ci-cd/pipeline_overview.md)
  - [CI/CD トラブルシューティング](../ci-cd/troubleshooting.md)

このドキュメント内では、CI/CD による品質ゲートが PR マージ前の必須要件であることのみを扱います。詳細は上記ドキュメントを参照してください。
